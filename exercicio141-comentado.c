...
tipo_lista* intersecta(tipo_lista *lista1, tipo_lista *lista2){
    ...
    lista3->fim = NULL;//inicializa o final da lista como null. (Usa setinha pois a variável é um ponteiro)

    tipo_no *ptr_atual1 = lista1->inicio, *ptr_atual2 = lista2->inicio, *ptr_atual3 = lista3->inicio, *ptr_novo = NULL;//cria variáveis para percorrer as listas e inicializa-as
    int cont = 0;


    while (ptr_atual1 != NULL){//percorrer a lista1 ate o final(prende os elementos da primeira lista)
        while(ptr_atual2 != NULL){//percorrer a lista 2 até o final(percorre os elementos da segunda para comparar)
            if(ptr_atual1->numero == ptr_atual2->numero){//caso o elemento da listaa 1 seja igual ao da lista 2
                    if(lista3->inicio == NULL){//verifica se é o primeiro elemento a ser inserido na lista 3
                        ptr_novo = (tipo_no*)malloc(sizeof(tipo_no));//aloca memoria para inserir o elemento
                        ptr_novo->numero = ptr_atual1->numero;//faz o campo de número receber o elemento da lista 1
                        ptr_novo->proximo = NULL;//campo proximo do novo elemento aponta para NULL, já que não tem mais nenhum elemento
                    ...
                    else{//caso a lista 3 já tenha elementos inseridos
                        while(ptr_atual3 != NULL){//percorre a terceira lista até o final
